...
The Anticcpcoin Core daemon can be configured to act as a trusted "border
router", implementing the anticcpcoin wire protocol and relay, making
...
The ZeroMQ feature in Anticcpcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building anticcpcoind:
...
For instance:
    ...
    $ anticcpcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/anticcpcoind.tx.raw \
...
These options can also be provided in anticcpcoin.conf.
...
From the perspective of anticcpcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into anticcpcoind directly. Furthermore, no information is
...
using. Anticcpcoind appends an up-counting sequence number to each
