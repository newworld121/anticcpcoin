...
const char * const BITCOIN_CONF_FILENAME = "anticcpcoin.conf";
...
#else
    const char* pszModule = "anticcpcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Anticcpcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Anticcpcoin";
#else
    // Unix-like
    return pathRet / ".anticcpcoin";
#endif
#endif
}
